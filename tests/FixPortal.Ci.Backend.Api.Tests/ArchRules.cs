...
public static class FixPortalArchRules
{
    ...
    public static IArchRule InterfacesMustHaveIPrefix() => Interfaces().Should().HaveNameStartingWith("I");
    ...
    public static IArchRule ExceptionsMustInheritFromException() =>
        Classes().That().HaveNameEndingWith("Exception").Should().BeAssignableTo(typeof(Exception));
    ...
    public static IArchRule AsyncMethodsMustEndInAsync() =>
        MethodMembers()
            .That()
            .HaveAnyAttributes(typeof(AsyncStateMachineAttribute))
            .Should()
    ...
    public static IArchRule ModelTypesMustBeSealed(string modelNamespace) =>
        Classes().That().ResideInNamespace(modelNamespace).Should().BeSealed();
    ...
    public static IArchRule NamespaceSlicesMustBeFreeOfCycles(string rootPattern) =>
        SliceRuleDefinition.Slices().Matching(rootPattern).Should().BeFreeOfCycles();

    /// <summary>Types in <paramref name="layer"/> must not depend on any type in <paramref name="forbidden"/>.</summary>
    public static IArchRule LayerMustNotDependOn(IObjectProvider<IType> layer, IObjectProvider<IType> forbidden) =>
        Types().That().Are(layer).Should().NotDependOnAny(forbidden);
}
