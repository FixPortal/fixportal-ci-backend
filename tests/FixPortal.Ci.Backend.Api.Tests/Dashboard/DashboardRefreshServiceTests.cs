...
public class DashboardRefreshServiceTests
{
    ...
    private static RepositorySnapshot Repo(string name, params (SignalState State, string? Conclusion)[] wfs) =>
        new(name, $"https://github.com/FixPortal/{name}", true,
            wfs.Select((w, i) => new WorkflowSnapshot($"wf{i}", $"wf{i}.yml", w.State,
                w.Conclusion is null ? null
                    : new WorkflowRun("completed", w.Conclusion, "url", "title", 1, "main", "push", Instant.MinValue))).ToList(),
            [], null, null, null);
    ...
    {
        var prior = new DashboardSnapshot(Instant.MinValue, "FixPortal",
            [Repo("a", (SignalState.Success, "success"))], [], null);
    ...
    {
        var withPr = Repo("a", (SignalState.Success, "success"), (SignalState.Failure, "failure"))
            with
    ...
    public void BuildSummary_tolerates_null_pull_requests_from_restored_old_snapshot()
    {
        ...
        var restored = Repo("old", (SignalState.Success, "success")) with { PullRequests = null! };
        var summary = DashboardRefreshService.BuildSummary([restored])
    ...
    {
        var withDeploys = Repo("a", (SignalState.Success, "success"))
            with
        {
            Deploys = [
        ...
        var summary = DashboardRefreshService.BuildSummary([withDeploys, nullDeploys])
            .ToDictionary(s => s.Key, s => s.Count);
    ...
    {
        var withFailingPackage = Repo("a", (SignalState.Success, "success"))
            with
        {
            Packages = [
        ...
        var summary = DashboardRefreshService.BuildSummary([withFailingPackage, nullPackages])
            .ToDictionary(s => s.Key, s => s.Count);
    ...
    {
        var fp = Repo("fixportal-engine", (SignalState.Success, "success"))
            with
        { Metrics = new RepoMetrics(1000, 2.0, 50, 1, Instant.MinValue) };
        var qfn = Repo("fixportal-quickfixn", (SignalState.Success, "success"))
            with
        { Metrics = new RepoMetrics(250, 1.0, 10, 0, Instant.MinValue) };
        var summary = DashboardRefreshService.BuildSummary([fp, qfn, Repo("c")])
            .ToDictionary(s => s.Key, s => s.Count);
    ...
    public void BuildCiTrend_marks_an_hour_passing_when_runs_present_and_none_failed()
    {
        ...
        var trend = DashboardRefreshService.BuildCiTrend(
            [RunAt(now - Duration.FromMinutes(20), "success")], now);
    ...
    public void BuildCiTrend_quiet_hours_after_a_failure_are_NoData()
    {
        ...
        var trend = DashboardRefreshService.BuildCiTrend(
            [RunAt(now - Duration.FromHours(5) - Duration.FromMinutes(10), "failure")], now);
    ...
    public void BuildCiTrend_quiet_hours_after_a_success_are_NoData()
    {
        var now = Instant.FromUtc(2026, 5, 30, 12, 0);
        var trend = DashboardRefreshService.BuildCiTrend(
            [RunAt(now - Duration.FromHours(5) - Duration.FromMinutes(10), "success")], now);
    ...
    public void BuildCiTrend_bins_runs_by_clock_hour_stable_across_intra_hour_refreshes()
    {
        ...
        var earlyFailing = early.Select((b, i) => (b, i)).Where(x => x.b.State == CiTrendState.Failing).Select(x => x.i);
    ...
    public void BuildCiTrend_hours_without_runs_are_NoData()
    {
        ...
        var trend = DashboardRefreshService.BuildCiTrend(
            [RunAt(now - Duration.FromMinutes(20), "success")], now);
        _ = trend[23].State.Should().Be(CiTrendState.Passing);
        _ = trend.Select((b, i) => new { b.State, i })
            .Should().OnlyContain(x => x.i == 23 || x.State == CiTrendState.NoData);
    }
    ...
    public void BuildCiTrend_excludes_runs_outside_the_24h_window()
    {
        ...
        _ = DashboardRefreshService.BuildCiTrend(runs, now)
            .Should().OnlyContain(b => b.State == CiTrendState.NoData);
    }
    ...
    public void BuildCiTrend_run_older_than_24h_produces_all_NoData()
    {
        ...
        var runs = new[]
        {
            RunAt(now - Duration.FromHours(25), "failure"),
        };
        _ = DashboardRefreshService.BuildCiTrend(runs, now)
            .Should().OnlyContain(b => b.State == CiTrendState.NoData);
    }
    ...
    public void BuildCiTrendForRefresh_preserves_previous_bucket_history_when_refresh_is_degraded()
    {
        ...
        var previousTrend = DashboardRefreshService.BuildCiTrend(
            [RunAt(now - Duration.FromHours(2), "failure")], now - Duration.FromMinutes(1));
    ...
    public async Task PersistAndPublishAsync_updates_live_state_even_when_persistence_fails()
    {
        ...
        _ = store.SaveAsync(Arg.Any<DashboardSnapshot>(), Arg.Any<CancellationToken>())
    ...
    public async Task PersistAndPublishAsync_propagates_cancellation_without_publishing_live_state()
    {
        ...
        var act = () => DashboardRefreshService.PersistAndPublishAsync(
            store,
            state,
            snapshot,
            snapshot,
    ...
    public void InheritEnrichment_does_not_overwrite_fresh_enrichment()
    {
        ...
        var result = DashboardRefreshService.InheritEnrichment(
            [Repo("a") with { Metrics = freshMetrics, Deploys = freshDeploys }], previous);
    ...
    public void InheritEnrichment_prefers_cached_merged_pr_over_previous_snapshot()
    {
        ...
        var previous = new DashboardSnapshot(Instant.MinValue, "FixPortal",
            [Repo("a") with { LastMergedPr = stale }], [], null);
        var mergedPrs = new PerRepoCache<MergedPullRequest>();
        mergedPrs.Update("a", cached);
