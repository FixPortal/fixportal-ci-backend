...
public class GitHubMergedPrTests
{
    private sealed class RecordingHandler(params string[] responses) : HttpMessageHandler
    {
        ...
        protected override Task<HttpResponseMessage> SendAsync(HttpRequestMessage request, CancellationToken cancellationToken)
        {
            ...
            {
                Content = new StringContent(_responses.Count > 0 ? _responses.Dequeue() : "[]", Encoding.UTF8, "application/json"),
            });
        }
    }
    ...
    public async Task GetLastMergedPullRequest_uses_search_api_with_is_merged_filter()
    {
        ...
        var body = """{"items":[{"number":42,"title":"Ship it","user":{"login":"alice"},"html_url":"https://github.com/FixPortal/repo/pull/42","pull_request":{"merged_at":"2026-05-30T11:00:00Z"}}]}""";
    ...
    private static string SearchPage(params (int Number, long? Merged, long Updated)[] items)
    {
        ...
        {
            var pr = i.Merged is { } m
                ? $$"""{"merged_at":"{{InstantPattern.ExtendedIso.Format(At(m))}}"}"""
                : "null";
    ...
    public async Task GetLastMergedPullRequest_accumulates_max_merge_across_pages()
    {
        ...
        var page1 = SearchPage(Enumerable.Range(0, SearchPageSize)
