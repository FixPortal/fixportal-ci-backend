...
- **Azure:** CI runs `az acr build` to produce the image, imports it to ACR, then
  ships `deploy/bicep/main.bicep` to **Azure Container Apps** (single always-on
...
- **CI quality gates:** `ci.yml` (build, test, CodeQL, GHCR publish, ACA deploy),
  `mutation.yml` (Stryker mutation testing), Dependabot.
